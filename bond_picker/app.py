...
def read_excel(path: Path):
    """读取 Excel 并返回 (债券列表, N1单元格日期)"""
    wb = load_workbook(path, read_only=True, data_only=True)
    ws = wb[SHEET_NAME]
    
    # 读取 N1 单元格作为数据日期
    n1_value = ws.cell(row=1, column=14).value  # column 14 = N
    data_date = ""
    if n1_value:
        if isinstance(n1_value, str):
            data_date = n1_value.strip()
        else:
            data_date = str(n1_value)
    ...
    print(f"读取 {len(bonds)} 条债券数据，跳过 {skipped} 条无效行，数据日期: {data_date}")
...
def load_data(excel_path=None):
    ...
    if not path.exists():
        ...
        return
    bonds, data_date = read_excel(path)
    BONDS_CACHE = bonds
    DATA_TIMESTAMP = data_date if data_date else datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print(f"数据加载完成: {len(BONDS_CACHE)} 条，日期: {DATA_TIMESTAMP}")
